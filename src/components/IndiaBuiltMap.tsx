      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full block overflow-visible">
        ...
        <g className="opacity-95">
          {dots.map((dot, index) => (
            <circle
              ...
              cy={dot.y}
              r={1.25}
              className="fill-[#e8e8e8] animate-dot-appear animate-dot-flicker"
        ...
        <g>
          {projectedHubs.map((hub) => (
            <g key={hub.name} className="cursor-pointer transition-transform duration-300" transform={`translate(${hub.x}, ${hub.y})`}>
              ...
              <circle
                r={13}
                className="fill-[#151515] stroke-[#F07F25] [stroke-width:1] transition-all duration-300"
              />
              <circle
                r={8}
                className="fill-[#151515] stroke-[#F07F25] [stroke-width:1.5] transition-all duration-300"
              />
              <circle
                r={4}
              ...
              <text
                y={-22}
                textAnchor="middle"
                className="fill-[rgba(255,255,255,0.7)] [font-family:Inter,Arial,sans-serif] text-[9px] font-semibold [letter-spacing:0.04em] [pointer-events:none] uppercase"
              >
                {hub.name}
              </text>
            </g>
        ...
        <g className="cursor-pointer" transform="translate(450 620)">
          ...
          <text
            y={-5}
            textAnchor="middle"
            className="fill-white [font-family:Inter,Arial,sans-serif] text-[17px] font-bold [letter-spacing:1px] max-md:text-[15px] max-sm:text-[13px]"
          >
            INDIA
          </text>
          <text
